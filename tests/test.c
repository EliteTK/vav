...
{
	float result = vec_dot(vec(3, 10, -4, 2), vec(3, 4, 2, 12));
...
{
	float *result = vec_cross(vec(3, 3, 5, 4), vec(3, -2, 1, 3));
...
{
	float result = vec_len(vec(3, 2, -2, 1));
...
{
	float *result = vec_addm(vec(3, 7, 3, 0), vec(3, 1, -5, 3), 3);
